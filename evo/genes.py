...
class PneuvoxBaseGene(object):
    ...
    def crossover(self, gene2):
        """ Creates a new gene by randomly choosing one of its parents in full."""
        ...
        if random() > 0.5:
            for a in self._gene_attributes:
                setattr(new_gene, a.name, getattr(self, a.name))
            new_gene.parent = self.parent
            new_gene.associations = self.associations
        else:
            for a in self._gene_attributes:
                setattr(new_gene, a.name, getattr(gene2, a.name))
            new_gene.parent = gene2.parent
            new_gene.associations = gene2.associations

        return new_gene


class PneuvoxNodeGene(PneuvoxBaseGene):
    ...
    def __init__(self, key):
        ...
        PneuvoxBaseGene.__init__(self, key)
...
class PneuvoxConnectionGene(PneuvoxBaseGene):
