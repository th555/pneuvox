    ...
    def run(self, fitness_function, constraint_function=None, n=None):
        ...
        while n is None or k < n:
            ...
            if constraint_function is not None:
                ...
                if k==1: # Reset all morphology_ages if it's the first time
                    for gen in self.population.values():
                        gen.morphology_age = 1

            # Evaluate all genomes using the user-provided function.
            fitness_function(list(self.population.items()), self.config, self.generation)


            # Ensure no singleton species
            """
            If a species ends up with only 1 member it cannot get out of this situation,
            this is especially bad combined with elitism as the singleton species will just
            keep on existing but not evolve.
            Exact duplication ensures that it will be assigned to the same species. Eventually
            it will mutate anyways.
            """
            speclens = {}
            for key, spec in self.species.species.items():
                ssize = len(spec.members)
                speclens[key] = ssize
                if ssize == 1:
                    parent = list(spec.members.values())[0]
